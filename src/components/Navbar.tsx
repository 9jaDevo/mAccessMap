import React, { useState } from 'react'; import { Link, useLocation } from 'react-router-dom'; import { Menu, X, MapPin, Award, User, LogOut, Settings, Trophy } from 'lucide-react'; import { useAuth } from '../contexts/AuthContext';
...
<img src="/logo.png" alt="mAccessMap Logo" className="w-8 h-8 object-contain" onError={(e) => { // Fallback to icon if image fails to load const target = e.target as HTMLImageElement; target.style.display = 'none'; const fallback = document.createElement('div'); fallback.className = 'w-8 h-8 bg-gradient-to-br from-emerald-500 to-blue-600 rounded-lg flex items-center justify-center'; fallback.innerHTML = ''; target.parentNode?.insertBefore(fallback, target); }} /> mAccessMap
